...
- 应用 JAR：[v1.1.7](https://github.com/xingchenv2/OCX-worker/releases/tag/v1.1.7) Release
- 安装器与 CLI：[`installer-latest`](https://github.com/xingchenv2/OCX-worker/releases/tag/installer-latest) Release（`install.sh`、`ocx`）
- systemd 服务名均为 `ocx-worker`，与历史手动部署路径兼容；若目录 `/opt/ocx-worker` 已存在，`install.sh` 会识别为升级模式
...
```bash
curl -fsSL https://github.com/xingchenv2/OCX-worker/releases/download/v1.1.7/install.sh -o /tmp/install.sh && sudo bash /tmp/install.sh
```

一行安装：

```bash
curl -fsSL https://github.com/xingchenv2/OCX-worker/releases/download/v1.1.7/install.sh | sudo bash
```
