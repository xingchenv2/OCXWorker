...
readonly JAR_RELEASE_TAG="latest"   # resolved dynamically by resolve_latest_tag()
...
# -----------------------------------------------------------------------------
# Resolve the real tag name for the "latest" release (GitHub API)
# The word "latest" is only a query alias in the API; it is NOT a valid
# download tag.  We must discover the actual tag (e.g. v1.1.7) first.
# -----------------------------------------------------------------------------
resolve_latest_tag() {
    local api_url="https://api.github.com/repos/${REPO}/releases/latest"
    local raw
    raw="$(curl -fsSL --max-time 15 "${api_url}" 2>/dev/null)" || true
    if [ -n "${raw}" ]; then
        # Extract tag_name – works even without jq
        local tag
        tag="$(echo "${raw}" | grep -m1 '"tag_name"' | sed 's/.*"tag_name"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/')"
        if [ -n "${tag}" ]; then
            echo "${tag}"
            return 0
        fi
    fi
    # Fallback: try the tags list endpoint
    raw="$(curl -fsSL --max-time 15 "https://api.github.com/repos/${REPO}/tags?per_page=1" 2>/dev/null)" || true
    if [ -n "${raw}" ]; then
        local tag
        tag="$(echo "${raw}" | grep -m1 '"name"' | sed 's/.*"name"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/')"
        if [ -n "${tag}" ]; then
            echo "${tag}"
            return 0
        fi
...
# Also resolve the JAR asset name dynamically so we don't hard-code the version
resolve_latest_jar_asset() {
    local api_url="https://api.github.com/repos/${REPO}/releases/latest"
    local raw
    raw="$(curl -fsSL --max-time 15 "${api_url}" 2>/dev/null)" || true
    if [ -n "${raw}" ]; then
        local name
        name="$(echo "${raw}" | grep -o '"name":"ocx-worker-[^"]*\.jar"' | head -1 | sed 's/"name":"//;s/"$//')"
        if [ -n "${name}" ]; then
            echo "${name}"
            return 0
        fi
...
# -----------------------------------------------------------------------------
# Resolve latest release info (tag + asset name) before downloads
# -----------------------------------------------------------------------------
RESOLVED_TAG=""
RESOLVED_JAR_ASSET=""
maybe_resolve_latest() {
    # Only resolve when the tag is the symbolic "latest"
    if [ "${JAR_RELEASE_TAG}" = "latest" ]; then
        info "查询 GitHub 最新版本 Release..."
        RESOLVED_TAG="$(resolve_latest_tag)" || true
        if [ -z "${RESOLVED_TAG}" ]; then
            warn "无法解析最新 release tag，将使用字面值 'latest'（可能 404）"
            RESOLVED_TAG="latest"
        else
            info "最新 Release tag: ${RESOLVED_TAG}"
        fi
        RESOLVED_JAR_ASSET="$(resolve_latest_jar_asset)" || true
        if [ -z "${RESOLVED_JAR_ASSET}" ]; then
            warn "无法解析最新 JAR 资产名，将使用默认值 ${JAR_ASSET}"
            RESOLVED_JAR_ASSET="${JAR_ASSET}"
        else
            info "最新 JAR 资产: ${RESOLVED_JAR_ASSET}"
        fi
    else
        RESOLVED_TAG="${JAR_RELEASE_TAG}"
        RESOLVED_JAR_ASSET="${JAR_ASSET}"
    fi
}
...
download_jar() {
    info "下载 JAR（Release：${RESOLVED_TAG}）…"
    local url tmp size attempt max
    url="https://github.com/${REPO}/releases/download/${RESOLVED_TAG}/${RESOLVED_JAR_ASSET}"
    info "下载: ${url}"
    ...
    while [ "${attempt}" -lt "${max}" ]; do
        ...
        if [ "${attempt}" -ge "${max}" ]; then
            err "JAR 下载失败"
            err "若出现 404，多为刚推送代码、或 GitHub Release 正更新，请过几分钟再试，并在仓库 Releases 页确认「${RESOLVED_TAG}」下已有 ${RESOLVED_JAR_ASSET}。"
            return 1
        fi
...
install_ocx_cli() {
    ...
    if [ -z "${src}" ]; then
        ...
        if download_with_retry "https://github.com/${REPO}/releases/download/${RESOLVED_TAG}/ocx" "${tmp}"; then
            src="${tmp}"
